...
$classQuery = "
    ...
    WHERE class_grade = '$gradeFilter' 
    AND class_section = '$sectionFilter'
    LIMIT 1";
...
$sql = "SELECT DISTINCT MONTH(date) AS month_num FROM archived_attendance WHERE studentID IS NOT NULL";
...
if ($result->num_rows > 0) {
    while ($rowData = $result->fetch_assoc()) {
        ...
        $dayCountSql = "SELECT COUNT(DISTINCT DAY(date)) AS day_count FROM archived_attendance WHERE MONTH(date) = $monthNum";
...
$sql = "SELECT studentID, name FROM archived_student WHERE class_id = $class_id ORDER BY name ASC";
$result = $conn->query($sql);
$row = 4; 
...
$totalDistinctDays = 0;
$distinctDaysSql = "SELECT COUNT(DISTINCT DATE(date)) AS distinct_day_count FROM archived_attendance WHERE studentID IS NOT NULL";
